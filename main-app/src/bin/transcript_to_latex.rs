...
fn print_transcript<T:Copy+ToString+Eq+Ord+Sub<Output=T>+Clone+Display+FromStr+Zero>(transcript:TranscriptWithMetadata<T>,opt:&Opts) {
    ...
    let use_exhausted : bool = opt.candidates.is_none() && transcript.transcript.counts.iter().any(|v|(!v.status.tallies.exhausted.is_zero()) && v.status.papers.exhausted.0>0);
...
fn main() -> anyhow::Result<()> {
    ...
    if let Ok(votes) = { let file = File::open(&opt.file)?; serde_json::from_reader::<_,ElectionData>(file) } {
        ...
        print_transcript(transcript,&opt);
    } else if let Ok(transcript) = { let file = File::open(&opt.file)?; serde_json::from_reader::<_,TranscriptWithMetadata<FixedPrecisionDecimal<6>>>(file) } {
        print_transcript(transcript,&opt);
    } else {
