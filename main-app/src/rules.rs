...
impl Rules {
        ...
        let transcript = match self {
            ...
            _ => { // handle 6 digit transcripts.
                let transcript = match self {
                    ...
                    Rules::beSTV => distribute_preferences_with_extractors::<beSTV>(data, candidates_to_be_elected, excluded_candidates, ec_resolutions, vote_types, print_progress_to_stdout, randomness, extractors, include_list_of_votes_in_transcript),
    ...
    pub fn find_changes(&self,data:&ElectionData,options:&ChangeOptions,verbose:bool) -> anyhow::Result<PossibleChanges> {
        Ok(match self {
            ...
            Rules::beSTV => PossibleChanges::SixDigitDecimals(options.find_changes::<beSTV>(data, verbose)?),
...
impl RulesDetails {
    pub fn list() -> Vec<RulesDetails> {
        vec![
            ...
            RulesDetails{ name: "BESTV".to_string(), description: "BESTV rules, according to the STV Margin Paper.".to_string() },
